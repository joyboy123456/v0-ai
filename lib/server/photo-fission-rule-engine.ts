...
1. 每段 imagePrompt 中，不要写"脸型五官延续参考图"或"人物脸部严格延续参考图"。
   改为写："面部全部特征（脸型+五官）和发色倾向以最后一张人像小卡（图${faceIdImageIndex}）为唯一基准。"

2. "参考图"三个字在脸部/五官/脸型语境下，特指图${faceIdImageIndex}人像小卡，不再指向图1主图。
   但图${faceIdImageIndex}只提供脸部核心特征和发色倾向，不提供发型轮廓、刘海、头发长度、帽子、发饰、服装或穿搭。
   图1主图提供穿搭比例、发型轮廓、刘海、头发长度、图1已有配饰、服装细节、场景与光线。

3. 不要在 imagePrompt 里写"不改变脸部特征"或"脸部保持一致"等含糊指令——
   必须明确写出"脸型形状、下颌线、颧骨、眼形、鼻型、嘴形、眉形、耳朵形状、面部三庭五眼比例和发色倾向全部以图${faceIdImageIndex}人像小卡为准"。

4. 推荐在每段 imagePrompt 中使用以下锚定句式：
   "TA面部全部特征（脸型+五官）和发色倾向严格以图${faceIdImageIndex}人像小卡为唯一权威参考，表情可自然变化但脸型骨骼结构和五官细节不改变；发型轮廓、刘海、发长和已有头部配饰仍以图1为准。"

5. 人像小卡相似度是脸部最高优先级：输出人物第一眼必须能被识别为图${faceIdImageIndex}人像小卡里的同一张脸，而不是只保留性别、年龄、肤色或大概气质。不要写"更像"、"接近"这类软要求，要写"同一张脸"、"可识别身份一致"、"脸型骨架和五官比例不改变"。

6. 外景镜头也不例外：场景换成蓝天草地时，脸型+五官仍然以图${faceIdImageIndex}人像小卡为准，不能因为换背景就改变脸。不要生成通用 AI 脸、网红脸、娃娃脸、过度美颜脸、成人化脸或"更漂亮但不像小卡"的脸。

7. 注意：图1主图的脸部核心区已被用户手动 mask 预处理覆盖，但发型轮廓、刘海、头发长度、图1已有头部配饰、手持包和服装穿搭仍然必须从图1读取并保留；图1没有的帽子、发饰、头饰、眼镜、耳饰或手持道具绝对不要新增。
`
}
