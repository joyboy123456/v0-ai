...
const PANTS_TEN_SHOT_BLUEPRINT: ReadonlyArray<PantsShotBlueprint> = [
  ...
  },
  {
    ...
    label: '左侧面姿势变化裤子商品图',
    description:
      '左侧面裤子商品图，身体朝向左侧约60°，允许45°-75°，画面边界、相机距离和人物大小严格参考主图，胸口以上区域不进入画面，画面上边缘与主图上边缘对齐；只执行本镜头后端注入的唯一指定姿势卡，让腿脚、脚掌落点和重心形成可见变化，手部是否出现严格服从主图露手证据，主图不露手则本镜头不露手，不改变裤长、版型和裤脚自然垂坠',
  },
  {
    ...PANTS_FOUR_SHOT_BLUEPRINT[1]!,
  ...
  },
  {
    ...
    label: '右侧面姿势变化裤子商品图',
    description:
      '右侧面裤子商品图，身体朝向右侧约60°，允许45°-75°，画面边界、相机距离和人物大小严格参考主图，胸口以上区域不进入画面，画面上边缘与主图上边缘对齐；只执行本镜头后端注入的唯一指定姿势卡，让腿脚、脚掌落点和重心形成可见变化，手部是否出现严格服从主图露手证据，主图不露手则本镜头不露手，不改变裤长、版型和裤脚自然垂坠',
  },
  {
    ...PANTS_FOUR_SHOT_BLUEPRINT[2]!,
...
export function buildPantsPlannerUserPrompt(
  ...
  return `请按系统提示词的规则，为本次裤子裂变任务输出 ${resultCount} 段提示词 JSON。每个 shotId 必须包含 poseCardId（从姿势库选择的姿势卡 id）和 imagePrompt（商品展示描述）。同一批先服从规定方向；姿势由你从姿势库选择，后端会校验合法性。imagePrompt 只负责描述当前方向、参考图证据和商品展示，不要写姿势细节。不得改变主图裤长基准、版型和裤脚宽度，也不得为了展示鞋而拉短或提起裤脚；但必须允许指定姿势带来自然裤脚垂坠、褶皱和鞋脚遮挡变化。${avoidSection}`
}
...
你是「AI 服装大片裂变」中裤子品类的镜头策划导演。用户上传 1 张裤子穿着主图，可选上传正面、侧面、背面平铺细节图。你负责在每个 shotId 固定的方向家族内安排商品展示与参考图证据边界，并从姿势卡库中为每个 shotId 选择一张姿势卡。你一次看到全部 ${resultCount} 个 shotId 的指令，必须从全局视角确保每张图选择的姿势在视觉上互不相似。人物状态只根据图1。

# 参考图变量边界

主图的画面边界、相机距离、人物大小、身体比例、腿长、上衣款式、颜色、图案、袖长、袖口、实际露出范围、鞋子款式、已有低位配饰、背景、光线和画面风格全部由图1读取。图1是否露手也是构图证据；Planner 不看图，不得擅自判断或安排露手例外，最终出图提示词会按“图1不露手则全批不露手；只有图1清楚露手才允许手部动作”处理。裤子的穿着版型、宽松度、裤长基准、裤脚宽度和整体廓形以图1穿着效果为准。图1的站姿、裤脚垂坠和裤脚与脚或鞋的原始遮挡关系不得覆盖你选择的姿势卡；脚或鞋可以被裤脚遮挡，不要求完整展示，指定腿部动作带来的自然褶皱、裤脚高度投影和鞋脚露出变化必须保留。
...
- 你必须从上方姿势卡库中为每个 shotId 选择一张姿势卡，将其 id 写入 poseCardId 字段。
- 选择的姿势卡方向必须与该 shotId 的方向家族一致：正面 shotId 只能选 front- 开头的卡，左侧选 left- 开头，右侧选 right- 开头，背面选 back- 开头。
- 你一次看到全部 ${resultCount} 个 shotId，必须从语义层面判断选择的姿势是否在视觉上相似。例如“脚尖点地”在正面和背面看起都是一脚尖点地另一脚承重，视觉相似，不应同时选择；“交叉步”和“开放三角”都是一腿跨出，视觉可能相似。
- 同一批 ${resultCount} 张图选择的姿势卡必须在视觉动作上互不相似：不同动作族、不同腿型轮廓、不同重心分布。不得仅靠方向变化或左右镜像来制造差异。
- 如果同一方向需要多张，例如 10 张里的 3 张正面、3 张左侧、3 张右侧，必须为同方向选择视觉差异最大的姿势卡。
...
只输出 JSON，格式必须是：
{"shots":[{"shotId":"shot_1","poseCardId":"front-soft-toe-out","role":"...","imagePrompt":"..."}]}
...
- 每段 imagePrompt 只描述当前方向家族、商品展示重点、参考图证据和画面气质；不要写腿部动作、重心、脚掌落点等姿势细节（姿势由 poseCardId 对应的姿势卡注入，不需要你在 imagePrompt 里重复）。
