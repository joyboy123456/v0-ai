...
export const PANTS_ACTION_CONTROL = [
  ...
  `- ${PANTS_FORBIDDEN_SIDE_HAND_SEAM}`,
  `- ${PANTS_FORBIDDEN_LOW_FRONT_HANDS}`,
  '- 手部允许双臂交叉、双手自然搭在腰间、双手完整藏在身后不出现，或一手明确动作搭配另一手隐藏；低位双手只有在与裤面保留明显空隙且不遮挡商品时才允许；禁止把手臂或手掌贴在裤缝、裤面、大腿外侧',
  '- 图1完全不露手时：无论 2 / 4 / 9 / 10 张，所有镜头都必须保持双手、手掌和手指完全不出现在画面；不再保留少量露手例外',
  '- 图1已经清楚露手时才可按姿势卡使用自然手势；无论是否露手，上衣款式、颜色、图案、袖长、袖口和可见范围都必须与图1一致',
  '- 禁止一只前臂横停腰前同时另一只手绕到身后，也禁止反手绕后腰同时另一臂横穿身前；人物最多两条手臂、两只手，肩膀—手肘—手腕链条必须符合人体结构，不得出现身后幽灵手、多余手掌、手臂穿模或反关节',
  '- 每张图都要服务裤子商品展示，动作幅度克制自然，不遮挡裤身主要可见区域；腿脚姿势必须执行唯一指定姿势卡，不能退回主图或侧面参考图里的普通站姿；不能改变裤长基准、版型或裤脚宽度，不能为了展示鞋而拉短或提起裤脚；允许明确腿部动作产生符合物理规律的自然褶皱、裤脚高度投影、鞋子露出程度和鞋脚遮挡变化',
  ...
  '- 构图边界、画面上边缘、相机距离和人物大小与主图一致；裤脚、脚和鞋是否露出以及彼此遮挡多少由唯一指定姿势和真实垂坠共同决定，不再完全复制主图站姿下的鞋脚遮挡，不要求脚或鞋完整展示',
  '- 胸口以上区域不进入画面，画面上边缘与主图上边缘对齐，上衣露出范围按主图',
  '- 最终出图每个 shotId 只能执行后端注入的一张唯一指定姿势卡；Planner 不得再写第二套姿势、相似姿势或库外替代动作',
  ...
  '- 托物、持物、撩发、扶头仍只保留为条件姿势灵感；主图没有对应物件、头发范围时不能新增。台阶、栏杆、椅子、台面只允许通过后端指定的受控支撑物姿势低频出现，不能由 Planner 自行新增或改款',
  ...
  '- 不强制手部动作族逐张互斥；自然安全的双手搭腰、双手藏于身后可以配合不同腿部动作复用，但手部与腿部的联合姿势不能重复',
...
export const PANTS_STYLE_ANCHOR = [
  ...
  '- 整体像可直接上架的裤子电商商品图：主图裁切距离明确、背景低存在感、裤装真实状态清楚',
...
export const PANTS_NEGATIVE_ADDON = [
  ...
  '- 胸口以上区域不进入画面，画面上边缘与主图上边缘对齐，上衣露出范围按主图；输出下边缘按主图，裤脚和鞋脚可见范围允许随唯一指定姿势自然变化',
  '- 主图已有的上衣、鞋子按同款保留，主图露出多少上衣就保留多少上衣；主图没有的不新增，也不要让新增内容遮挡裤身主要商品区域',
  '- 不要让侧面镜头角度不清、左右侧重复、背面变侧面、裤身扭曲、左右腿比例失真，或擅自改变裤长基准、裤脚宽度和裤型；不要把唯一指定姿势退化成参考图普通站姿',
...
function getPantsPoseShapeGroup(
  ...
  const sidePrefix = view === 'left' ? 'left' : view === 'right' ? 'right' : 'side'
  if (/low-hands-away-cross-step/.test(id)) return `${sidePrefix}-cross-step`
  if (/knee-lift-toe-touch/.test(id)) return `${sidePrefix}-knee-bend`
  if (/walking-pause|pure-side-walk/.test(id)) return `${sidePrefix}-walking`
  if (/front-foot-lift/.test(id)) return `${sidePrefix}-front-foot-lift`
  if (/heel-touch-toe-up/.test(id)) return `${sidePrefix}-heel-touch-toe-up`
  if (/toe-point-back|front-toe-point-long-line|white-rail-toe-point/.test(id)) {
    return `${sidePrefix}-toe-point`
  }
  if (/compact-toe-out|compact-staggered-v/.test(id)) {
    return `${sidePrefix}-compact-foot`
  }
  if (/pure-side-staggered|one-waist-one-behind|all-heels-staggered|hands-hidden-open-step|side-reach-one-waist/.test(id)) {
    return `${sidePrefix}-side-staggered`
  }
  if (/hands-down-stable|waistband-one-hand|reverse-hand-rear-waist|both-hands-waistband-staggered/.test(id)) {
    return `${sidePrefix}-grounded-staggered`
  }
  return null
}
...
export function getPantsAssignedPoseForShot(
  ...
  validatePantsAssignedPosePlan(resultCount, drawSeed, mainHandVisibility)
...
const PANTS_FOUR_SHOT_BLUEPRINT: ReadonlyArray<PantsShotBlueprint> = [
  {
    label: '正面主图裁切距离裤子商品图',
    description:
      '正面裤子商品图，画面边界、相机距离和人物大小严格参考主图，胸口以上区域不进入画面，画面上边缘与主图上边缘对齐；模特正面面对镜头，可有0°-15°微左或微右变化，只展示参考图真实可见的正面状态，裤长基准、版型和裤脚宽度按主图，腿脚姿势按本镜头唯一指定姿势执行，垂坠状态及鞋脚遮挡可随姿势自然变化，已有上衣图案、鞋子和低位配饰保持不变',
    view: 'front',
    scene: 'reference',
  },
  {
    label: '左侧面主图裁切距离裤子商品图',
    description:
...
const PANTS_TEN_SHOT_BLUEPRINT: ReadonlyArray<PantsShotBlueprint> = [
  ...
  PANTS_FOUR_SHOT_BLUEPRINT[0]!,
  {
    ...
    label: '正面姿势变化裤子商品图',
    description:
      '正面裤子商品图，画面边界、相机距离和人物大小严格参考主图，胸口以上区域不进入画面，画面上边缘与主图上边缘对齐；只执行本镜头后端注入的唯一指定姿势卡，让腿脚、脚掌落点和重心形成可见变化，手部是否出现严格服从主图露手证据，主图不露手则本镜头不露手，不改变裤长、版型和裤脚自然垂坠',
  },
  {
    ...PANTS_FOUR_SHOT_BLUEPRINT[0]!,
    label: '正面自然站姿主图裁切距离裤子商品图',
    description:
      '正面裤子商品图，画面边界、相机距离和人物大小严格参考主图，胸口以上区域不进入画面，画面上边缘与主图上边缘对齐；手部和腿部严格执行本镜头指定姿势卡，不遮挡裤身主要区域，只展示参考图真实可见的正面状态，裤子版型与裤脚宽度按主图',
  },
  {
    ...PANTS_FOUR_SHOT_BLUEPRINT[1]!,
    label: '左侧面主图裁切距离裤子商品图',
    description:
      '左侧面裤子商品图，身体朝向左侧约30°，允许15°-45°，与同批其它左侧镜头形成可见角度差；画面边界、相机距离和人物大小严格参考主图，胸口以上区域不进入画面，画面上边缘与主图上边缘对齐；只展示参考图真实可见的左侧状态，手部和腿部严格执行本镜头指定姿势卡，不遮挡裤身主要区域，裤长与版型严格按主图',
  },
  {
    ...PANTS_FOUR_SHOT_BLUEPRINT[1]!,
    label: '左侧面姿势变化裤子商品图',
    description:
      '左侧面裤子商品图，身体朝向左侧约60°，允许45°-75°，与同批其它左侧镜头形成可见角度差；画面边界、相机距离和人物大小严格参考主图，胸口以上区域不进入画面，画面上边缘与主图上边缘对齐；只执行本镜头后端注入的唯一指定姿势卡，让腿脚、脚掌落点和重心形成可见变化，手部是否出现严格服从主图露手证据，主图不露手则本镜头不露手，不改变裤长、版型和裤脚自然垂坠',
  },
  {
    ...PANTS_FOUR_SHOT_BLUEPRINT[1]!,
    label: '左侧面稳定站姿主图裁切距离裤子商品图',
    description:
      '左侧面裤子商品图，身体朝向左侧约90°，允许75°-95°，最高不超过95°，与同批其它左侧镜头形成可见角度差；画面边界、相机距离和人物大小严格参考主图，胸口以上区域不进入画面，画面上边缘与主图上边缘对齐；模特身体保持明确左侧角度，手部和腿部严格执行本镜头指定姿势卡，只展示参考图真实可见的左侧状态，裤长、裤型和裤脚宽度按主图',
  },
  {
    ...PANTS_FOUR_SHOT_BLUEPRINT[2]!,
    label: '右侧面主图裁切距离裤子商品图',
    description:
      '右侧面裤子商品图，身体朝向右侧约30°，允许15°-45°，与同批其它右侧镜头形成可见角度差；画面边界、相机距离和人物大小严格参考主图，胸口以上区域不进入画面，画面上边缘与主图上边缘对齐；只展示参考图真实可见的右侧状态，手部和腿部严格执行本镜头指定姿势卡，裤长与版型严格按主图',
  },
  {
    ...PANTS_FOUR_SHOT_BLUEPRINT[2]!,
    label: '右侧面姿势变化裤子商品图',
    description:
      '右侧面裤子商品图，身体朝向右侧约60°，允许45°-75°，与同批其它右侧镜头形成可见角度差；画面边界、相机距离和人物大小严格参考主图，胸口以上区域不进入画面，画面上边缘与主图上边缘对齐；只执行本镜头后端注入的唯一指定姿势卡，让腿脚、脚掌落点和重心形成可见变化，手部是否出现严格服从主图露手证据，主图不露手则本镜头不露手，不改变裤长、版型和裤脚自然垂坠',
  },
  {
    ...PANTS_FOUR_SHOT_BLUEPRINT[2]!,
    label: '右侧面自然站姿主图裁切距离裤子商品图',
    description:
      '右侧面裤子商品图，身体朝向右侧约90°，允许75°-95°，最高不超过95°，与同批其它右侧镜头形成可见角度差；画面边界、相机距离和人物大小严格参考主图，胸口以上区域不进入画面，画面上边缘与主图上边缘对齐；模特保持明确右侧角度，手部和腿部严格执行本镜头指定姿势卡，不遮挡裤身主要区域，只展示参考图真实可见的右侧状态，裤长、裤型和裤脚宽度按主图',
  },
]
...
export function buildPantsPlannerUserPrompt(
  ...
  return `请按系统提示词的规则，为本次裤子裂变任务输出 ${resultCount} 段提示词 JSON。同一批先服从规定方向；最终出图会由后端注入唯一姿势卡，Planner 只负责描述当前方向、参考图证据和商品展示，不要创造第二套姿势、相似姿势或库外替代动作。不得改变主图裤长基准、版型和裤脚宽度，也不得为了展示鞋而拉短或提起裤脚；但必须允许唯一指定姿势带来自然裤脚垂坠、褶皱和鞋脚遮挡变化。${avoidSection}`
}
...
export function buildPantsPlannerSystemPrompt(
  ...
  validatePantsAssignedPosePlan(resultCount)
  const blueprint = getPantsShotBlueprintForCount(resultCount)
  const distribution =
    resultCount === 2
  ...
  const requiredShots = blueprint
    .map(
      (shot, index) => {
        ...
        return `${shotId}：${shot.label}。方向硬约束：${getPantsPoseDirectionRule(shot.view)} 运行时会对每个任务执行独立加权抽卡并注入唯一最终姿势；Planner 不得另写第二套动作，不得把任何高偏好姿势写成每批必出。${shot.description}。${buildPantsShotReferenceInstruction(shot, detailAvailability)}`
      },
    )
    .join('\n')
  const recentPoseRules = buildRecentPantsPoseRuleSection(recentActionHints)
  const poseLibrary = buildPantsPoseLibraryPrompt()

  return `# 角色

你是「AI 服装大片裂变」中裤子品类的镜头策划导演。用户上传 1 张裤子穿着主图，可选上传正面、侧面、背面平铺细节图。你负责在每个 shotId 固定的方向家族内安排商品展示与参考图证据边界；最终姿势由后端为每个 shotId 注入唯一指定姿势卡，你不能再创造第二套姿势或替代动作，也不能跨越该 shotId 的正面、左侧、右侧或背面方向。人物状态只根据图1。

# 参考图变量边界

主图的画面边界、相机距离、人物大小、身体比例、腿长、上衣款式、颜色、图案、袖长、袖口、实际露出范围、鞋子款式、已有低位配饰、背景、光线和画面风格全部由图1读取。图1是否露手也是构图证据；Planner 不看图，不得擅自判断或安排露手例外，最终出图提示词会按“图1不露手则全批不露手；只有图1清楚露手才允许手部动作”处理。裤子的穿着版型、宽松度、裤长基准、裤脚宽度和整体廓形以图1穿着效果为准。图1的站姿、裤脚垂坠和裤脚与脚或鞋的原始遮挡关系不得覆盖后端唯一指定姿势；脚或鞋可以被裤脚遮挡，不要求完整展示，指定腿部动作带来的自然褶皱、裤脚高度投影和鞋脚露出变化必须保留。
...
- 本次同一批 ${resultCount} 张图先服从规定的正面、左侧、右侧、背面方向家族；最终每张只执行后端注入的唯一姿势卡，通过视觉动作族、腿脚落点、重心和脚掌状态形成差异。
- 每个 shotId 已经分配唯一指定姿势卡，必须逐字落实该卡的手部、腿部、重心和脚掌落点，不得自行替换；指定姿势卡优先于“自然站立”“稳定站姿”等泛化措辞。
- ${PANTS_FORBIDDEN_BILATERAL_HANDS_DOWN}
- 如果同一方向需要多张，例如 10 张里的 3 张正面、3 张左侧、3 张右侧，优先使用不同手部动作、腿脚站位、重心和微侧幅度；同一方向仍必须保持为主要可见面。
- 姿势变化不能遮挡裤身主要商品区域，不能改变主图裤子版型、裤长基准、裤脚宽度或人物比例；指定姿势可以改变裤脚垂坠、自然褶皱、鞋子露出程度和鞋脚遮挡逻辑，不得为了复制参考图站姿而取消腿脚动作。
- Planner 不看图片，不能判断口袋、道具或头发是否存在；自动策划时不要直接选择依赖这些证据的条件互动，只能学习其肢体关系并改成安全动作。台阶、栏杆、透明金属椅子和台面只能在后端注入的唯一姿势卡明确写出时出现。
${recentPoseRules}
...
只输出 JSON，格式必须是：
{"shots":[{"shotId":"shot_1","role":"...","imagePrompt":"..."}]}

要求：
- shots 数量必须等于 ${resultCount}。
- shotId 必须从 shot_1 连续到 shot_${resultCount}。
- 每段 imagePrompt 必须是 160-280 字的自然语言段落，不要 markdown 标题、列表或条件分支。
- 每段必须写出该 shotId 的指定姿势卡 id、完整手部动作和完整腿部动作；不得输出与指定姿势卡冲突的第二套动作。
- 每段都要明确写“画面边界、相机距离和人物大小与主图一致，胸口以上区域不进入画面，画面上边缘与主图上边缘对齐，上衣露出范围按主图；裤长基准、裤型和裤脚宽度严格跟随主图，不要求脚或鞋完整展示，唯一指定姿势优先于参考图站姿，指定动作产生的自然褶皱、裤脚垂坠、鞋子露出程度与鞋脚遮挡变化符合物理规律”，并写清当前方向家族、指定姿势卡、裤子展示重点、主图已有上衣/鞋子/随身物条件保留、参考背景延续和真实商品图质感；只有指定姿势卡明确含受控支撑物时，才允许写该支撑物。
- 每段只允许使用对应 shotId 后写明的参考图编号与角度分工；缺失图片已经被跳过，不得出现任何未列出的细节图名称或错误编号。
- 每段都要说明“图案、贴布、刺绣、logo、拼接、口袋及其它商品结构只按参考图中真实可见状态复现；参考图没有就不要生成，也不要换腿、换面、镜像或移动位置”，不要列举或暗示参考图中没有证据的具体结构。
- 所有镜头都沿用参考图背景和光线，不生成蓝天草地、街景、建筑、人群或生活故事；除后端指定的白色台阶、白色栏杆、透明金属椅子或白色台面外，不新增其它场景物。
