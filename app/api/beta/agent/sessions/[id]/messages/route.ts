...
export async function POST(request: NextRequest, context: SessionRouteContext) {
  return agentBetaResponse(request, async (service, userId) => ({ session: await service.sendMessage(userId, (await context.params).id, await readAgentBetaBody(request)) }))
}
