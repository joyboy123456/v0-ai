...
**与外部 API 稳定性 spec 的边界**：`external-image-api-reliability.md` 管「单次 Google 调用怎么稳」（重试 / 限流 / 错误分类）；本 spec 管「N 次单调用怎么编排、失败怎么收口」。两者通过 `runGoogleImageEdit` 入参 `traceId / shotId` 接驳。
...
runXxxFissionPipeline({ taskId, inputImages, params, apiKey, timeoutMs, onShotResult, targetIds? })
   ...
   ├─ 2. concurrency = clamp(env, 1, items.length)，photo-fission 默认 3，pose-fission 默认 2
   ├─ 3. worker pool：N 个 worker 共享 nextIndex 游标
   │     ├─ 每个 worker 循环 `while (currentIndex < items.length)`
   │     ├─ runGoogleImageEdit({ traceId: `${taskId}_${shot|templateId}`, shotId, ... })
   ...
   ├─ 4. 全部 worker await Promise.all 后收口
...
| 触发 | 处理 | 后续 |
|---|---|---|
| 单 item `runGoogleImageEdit` throw | catch → 标 `error`，continue 下一个 | partial / failed 由 resolveTaskCompletion 决定 |
...
### Good — 全部成功
```
photo-fission 9 shot / concurrency=3 / IPM=10
→ 3 个 worker 抢占 shotPlan[0..8]
→ 每张：runGoogleImageEdit 成功 → onShotResult 写盘 → persistedResults.push
→ pipeline 返回 9 张
...
- **并发上限**：env `PHOTO_FISSION_CONCURRENCY=2` 启动 9 shot 任务，断言任一时刻 in-flight 不超过 2
...
const results = await Promise.all(
  shotPlan.map((shot) => runGoogleImageEdit({ ... }))
)
```
**为什么错**：
...
const worker = async () => {
  while (true) {
    ...
    try {
      const single = await runGoogleImageEdit({ traceId: `${taskId}_${shot.shotId}`, ... })
...
`runThirdPartyWorkflow` 是为支持 `IMAGE_API_PROVIDER=raycast | google | demo` 切换设计的；pose-fission 上线时 Google 是唯一可用 provider，且 raycast 路径不支持 pose-fission 所需的「锚定主图 + 改姿势」语义。直接在 `task-store.runTask` 内按 `isPoseFission` 分流到 `runPoseFissionPipeline` → `runGoogleImageEdit`，省一层 dispatcher、避免双重 fetch。
